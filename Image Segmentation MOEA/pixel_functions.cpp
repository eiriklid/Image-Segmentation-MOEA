...
point_vec_t neighbours(Point pixel, cv::Mat* image) {
	point_vec_t neighbour_array;
	if (pixel.x > 0) {
		neighbour_array.push_back(Point(pixel.x - 1, pixel.y));
	}
	if (pixel.y > 0) {
		neighbour_array.push_back(Point(pixel.x, pixel.y - 1));
	}
...
point_vec_t neighbours(Point pixel, cv::Mat* image, int boxes) {
			...
			for (int j = -boxes; j <= boxes; j++) {
				...
				if (i != 0 || j != 0) {
					...
					if ( ((pixel.x + i) >= 0 && (pixel.x + i) < image->cols) && ((pixel.y + j) >= 0 && (pixel.y + j)< image->rows)) {
						neighbour_array.push_back(Point(pixel.x + i, pixel.y + j));
